...
class GovernorAuditEngine:
    ...
    def __init__(self, test_mode: bool = False, test_path: str = None):
        if test_mode and test_path:
            ...
            self.genome_path = os.path.join(
                test_path, "docs/01_GENOME_DNA_CORE/LAWS/Schema_Genome_Core.json"
            )
            self.registry_path = os.path.join(
                test_path, "docs/02_PHENOTYPE_RUNTIME/Registry_Codon_Tasks.md"
            )
        else:
            # Mode Production
            self.genome_path = "docs/01_GENOME_DNA_CORE/LAWS/Schema_Genome_Core.json"
    ...
    def audit_tasks(self) -> Tuple[int, int]:
        ...
        print(
            f"   - Taux d'autonomie : {auto_validated/(auto_validated+blocked)*100:.1f}%"
        )
